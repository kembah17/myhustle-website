...
export default function NewAreaPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    else { const d = await res.json(); setError(d.error || 'Failed to create area') }
    setSaving(false)
  }

  return (
    <div className="max-w-lg">
      <h2 className="text-lg font-semibold text-hustle-dark mb-4">Add New Area</h2>
      <form onSubmit={handleSubmit} className="bg-white rounded-xl border border-gray-200 p-6 space-y-4">
        ...
        <div>
          ...
          <input type="text" required value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })}
            className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-hustle-blue focus:border-transparent" placeholder="e.g. Lekki" />
        </div>
        <div>
