...
export default function EditAreaPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [form, setForm] = useState({ name: '', city_id: '', lat: '', lon: '', description: '' })
  ...
  useEffect(() => {
    Promise.all([
      ...
      setCities(cts)
      const area = areas.find((a: { id: string }) => a.id === id)
      if (area) setForm({ name: area.name, city_id: area.city_id, lat: String(area.lat), lon: String(area.lon), description: area.description || '' })
      setLoading(false)
    })
  }, [id])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (res.ok) router.push('/dashboard/admin/areas')
    else { const d = await res.json(); setError(d.error || 'Failed to update area') }
    setSaving(false)
  }
  ...
  return (
    <div className="max-w-lg">
      <h2 className="text-lg font-semibold text-hustle-dark mb-4">Edit Area</h2>
