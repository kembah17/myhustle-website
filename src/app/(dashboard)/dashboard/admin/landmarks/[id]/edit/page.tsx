...
export default function EditLandmarkPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    Promise.all([
      fetch('/api/admin/landmarks').then(r => r.json()),
      fetch('/api/admin/cities').then(r => r.json()),
      fetch('/api/admin/areas').then(r => r.json()),
    ]).then(([landmarks, cts, ars]) => {
      setCities(cts)
      setAreas(ars)
      const lm = landmarks.find((l: { id: string }) => l.id === id)
      if (lm) setForm({
        name: lm.name, city_id: lm.city_id, area_id: lm.area_id || '',
        lat: String(lm.lat), lon: String(lm.lon), type: lm.type,
        radius_km: String(lm.radius_km), description: lm.description || '',
      })
      setLoading(false)
    })
  }, [id])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    else { const d = await res.json(); setError(d.error || 'Failed to update landmark') }
    setSaving(false)
  }
  ...
  return (
    <div className="max-w-lg">
      <h2 className="text-lg font-semibold text-hustle-dark mb-4">Edit Landmark</h2>
